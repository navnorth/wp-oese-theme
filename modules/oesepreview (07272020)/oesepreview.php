...
//require_once OESEPREVIEW_ROOT.'/settings.php';
...
function post_update_function( $post_ID, $post_after, $post_before ) {
  update_post_meta($post_ID, '_post_oesepreview_pwd', uniqid());      // generate password
  echo 'WOW:'.get_post_meta($post_ID, '_post_oesepreview_pwd', true);
}
...
function copy_post($post, $to=null, $parent_id=null, $status='draft') {
  ...
  if ($to) {
    ...
    // maintain original date. Fixes scheduled oesepreviews overwriting the date. see issue #9
...
function post_button() {
  ...
  <?php else: ?>
    ...
    <div id="oese-preview-draft-publish-warning"><em><?php echo sprintf(__('<strong style="font-size:14px;color:red;">WARNING</strong>: Publishing this oesepreview will overwrite %s.', 'oesepreview'), get_parent_editlink($parent, __('its original.', 'oesepreview')). ' and will go live.<br>Please use "Save Draft" if you only wish to save this oesepreview for review')?></em></div>
  <?php endif;
}
...
function post_status_label($states, $post) {
  if (!empty($post) && get_oesepreview_of($post)) {
    $label = is_original_post($post) ? __('Backup Revision', 'oesepreview') : __('Revision', 'oesepreview');
...
function notice() {
  ...
  <div class="notice notice-warning">
      <p><?php echo sprintf(__('Currently editing a oesepreview of %s. Publishing this post will overwrite it.', 'oesepreview'), get_parent_permalink($parent)); ?></p>
  </div>
  <?php
  endif;
}
...
function get_create_button_text() {
  ...
  if(metadata_exists('post', $post->ID, '_post_oesepreview_id')){
    $_oesepreview_id = get_post_meta($post->ID, '_post_oesepreview_id', true);
    if( is_null(get_post($_oesepreview_id))){
      $_btn = apply_filters('oesepreview_create_oesepreview_button_text', __('Revise', 'oesepreview'));
    }else{
      $_btn = apply_filters('oesepreview_create_oesepreview_button_text', __('Edit Revision', 'oesepreview'));
    }
  }else{
    $_btn = apply_filters('oesepreview_create_oesepreview_button_text', __('Revise', 'oesepreview'));
  }
